...
SITE="/Users/jackiexiao/code/OneApps/Sites/OneApps.Studio/apps/web/public/one-apps"
FONT="/System/Library/Fonts/Hiragino Sans GB.ttc"
ICON="$SITE/assets/onevoice-icon.png"
PHONE="$SITE/products/onevoice/onevoice-iphone-record.png"
HISTORY="$SITE/products/onevoice/onevoice-iphone-history.png"
MAC="$SITE/products/onevoice/onevoice-transcribe-file.png"
mkdir -p "$OUT"
...
poster "onevoice-a-private.png" '#111311' '#B8F36B' 'ONEVOICE · 免费 / 开源 / 离线' \
  $'你的声音，\n不该先上传\n再被听见。' \
  $'本地完成语音识别\n没有账号 · 没有广告\n没有 OneVoice 音频服务器' \
  '免费使用 · oneapps.studio/product/onevoice'

poster "onevoice-b-fast.png" '#12172A' '#7CCBFF' 'ONEVOICE · 任意 APP 全局语音输入' \
  $'打字跟不上想法，\n就别再打字。' \
  $'按住 Fn 开始说话\n松开，文字自动写入光标处\n离线也能正常工作' \
  'Mac 免费下载 · 开箱即用'

poster "onevoice-c-free.png" '#211416' '#FF9B73' 'ONEVOICE · 不按分钟计费' \
  $'语音输入，\n不该又是\n一笔订阅。' \
...
magick -size 1600x900 xc:'#0E110F' \
  ...
  "$OUT/onevoice-landscape-private.png"
...
magick -size 1080x1350 xc:'#111311' \
  ...
  "$OUT/onevoice-open-source.png"

# 竖屏短视频：三张海报依次出现，适合视频号、小红书、抖音和朋友圈。
ffmpeg -y \
  -loop 1 -t 4.6 -i "$OUT/onevoice-a-private.png" \
  -loop 1 -t 4.6 -i "$OUT/onevoice-b-fast.png" \
  -loop 1 -t 4.6 -i "$OUT/onevoice-c-free.png" \
